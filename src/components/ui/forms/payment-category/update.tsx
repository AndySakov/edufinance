...
export interface UpdatePaymentCategoryFormProps {
  paymentCategory?: BillType;
}

export function UpdatePaymentCategoryForm({
  paymentCategory,
}: UpdatePaymentCategoryFormProps) {
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    client
      .patch<ResponseWithOptionalData<BillType>>(
      ...
      .then((res) => {
        if (res.data.success) {
          const paymentCategory = data(res.data);
          if (paymentCategory) {
            window.location.reload();
            console.log("Payment category updated");
          } else {
            console.log("No data");
          }
        } else {
          console.log("Error updating payment category");
          toast({
            title: "Error",
      ...
      <div className="space-y-6 bg-white shadow-sm mx-auto p-6 rounded-lg max-w-3xl">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Update this payment category's details here.
          </p>
          <Form {...form}>
            <form
                    ...
                    <FormItem>
                      ...
                      <FormControl {...field}>
                        <Input
                          {...field}
                          id="name"
                          placeholder="Payment Category Name"
                        />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
